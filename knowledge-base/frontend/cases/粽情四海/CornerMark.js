...
class CornerMark extends React.PureComponent<any, any> {
  ...
  render() {
    ...
    if (!cornerMarkSetting || cornerMarkSetting.showType === 0 || count === 0) {
      ...
      return (
        <div className={'app-729087887109857281-toolbar-corner-mark'} >
          <span className={'app-729087887109857281-toolbar-corner-mark-dot'}></span>
        </div>
      );
    } else if (cornerMarkSetting.showType === 2) {
      return (
        <div className={'app-729087887109857281-toolbar-corner-mark'} >
          <span className={'app-729087887109857281-toolbar-corner-mark-count'}>{count > 99 ? '99+' : count}</span>
        </div>
      );
