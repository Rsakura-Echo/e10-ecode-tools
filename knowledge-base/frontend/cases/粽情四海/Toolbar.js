...
class Toolbar extends React.PureComponent {
  render() {
    ...
    return (
      <div className="app-729087887109857281-toolbar">
        <HomeIcon />
        <ImIcon />
        <AppMoreIcon />
        <div className="app-729087887109857281-toolbar-item-split" />
        <QuickSearchCom className="open app-729087887109857281-quick-search" />
        {/*
          {
            navMenus.map((item, index) => {
              if (item.menuKey === 'quick_create') {
                ...
                return <div key={index} className="app-729087887109857281-toolbar-item-split" />;
              }
            })
          }
        */}
        <div className="app-729087887109857281-toolbar-no-default-setting">
          {
            navMenus_show.map((item, index) => {
              if (item.menuType === 'DEFAULT_MENU') {
                  ...
                  return <div key={index} className={'app-729087887109857281-toolbar-item-split'}>|</div>;
                }
                return null;
            })
          }
  ...
  onOpen = (item) => {
    ...
    // }
    const LayoutSDK = window.weappLayout?.LayoutSDK;
    LayoutSDK?.utils?.quickMenuClick(item);
  }
}
