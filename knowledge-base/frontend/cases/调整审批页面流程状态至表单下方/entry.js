...
const CustWfStatus = React.forwardRef((props, ref) => {
  ...
  if (props.weId === "3rdcst_oxa9w7_i8bbvp_m9jfa0_csp16w_gfar3p_6xdqsn") {
    ...
    if (!dom) {
      execRender('.flowpageCardDetailForm', () => {
        ...
        const dom = document.querySelector('.ecode-cust-wf-status-dom');
        if (dom) return;
        const pdom = document.querySelector('.flowpageCardDetailForm');
        const div = document.createElement('div');
        div.setAttribute('class', 'ecode-cust-wf-status-dom');
        
        pdom.appendChild(div);
        ReactDOM.render(<CustWfStatus />,div);
      }); 
    }
