...
泛微 E10 ecode 低代码二开工具包 — 集成 **6 个 Claude Code Skill** + **262 个真实案例库** + **26 篇前后端开发文档** + **82 模块 5700+ 篇实施文档语义搜索**，覆盖"配置查询 → 前端二开 → 后端二开 → 知识库维护"全流程。
...
**工作流**：优先检索 262 个真实案例库（0.003s 定位）→ 有匹配案例则基于模板修改 → 无匹配则检索前端知识库 → 生成 entry.js + 组件 + CSS 完整代码。
...
### 6. `ecode-case-parser` — 真实案例 ZIP 入库

解析 E10 ecode 平台导出的真实案例 ZIP 文件，自动入库并重建智能索引。

```
触发示例：
  "把这个案例 ZIP 导入案例库 /path/to/case.zip"
  "批量导入 ~/Downloads/ 下所有案例 ZIP"
```

**工作流**：解压 ZIP → 解析 ecode.json → base64 解码源码 → 提取附件 → 按场景分类 → 重建 INDEX.md 智能索引。
...
```bash
# 导入单个案例 ZIP
python3 tools/parse_case_zip.py /path/to/case.zip

# 批量导入（目录下所有 ZIP）
python3 tools/batch_parse_cases.py /path/to/zip/dir

# 重建智能索引（按场景分类 + 技术标签 + 描述提取）
python3 tools/build_case_index.py
```

| 工具 | 用法 |
|------|------|
| `tools/parse_case_zip.py` | 解析单个 ZIP → 解码 base64 → 提取源码 + 附件 |
| `tools/batch_parse_cases.py` | 批量导入目录下所有 ZIP |
| `tools/build_case_index.py` | 扫描所有案例 → 提取 README + 代码标签 → 按 16 场景分类生成 INDEX.md |
...
│   ├── cases/                    # 真实案例库（262案例/16场景/539 JS文件）
│   │   ├── INDEX.md               # 智能索引（按场景分类+技术标签+描述）
│   │   └── REFERENCE.md           # 参考速查（全局变量+陷阱+ESB方式对比）
...
│   ├── parse_case_zip.py          # 案例 ZIP 解析
│   ├── batch_parse_cases.py       # 批量案例导入
│   ├── build_case_index.py        # 智能案例索引生成
...
1. **案例 ZIP 入库**：`/ecode-case-parser` 或 `python3 tools/parse_case_zip.py <zip>`，自动解码入 `cases/`
2. **URL 入库**：给 `ecode-knowledge-maintainer` skill 一个网址，自动抓取分类入库
3. **本地文件**：给 skill 一个本地 doc/pdf/md 文件路径，自动转换入库
4. **批量导入**：将官方 HTML 文档 ZIP 放入 `doc/html版/`，执行 `step1 → step2 → build_embeddings`
